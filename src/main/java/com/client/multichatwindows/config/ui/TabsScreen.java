...
public class TabsScreen extends ScrollableDarkScreen {
    ...
    protected void init() {
        ...
        int cx = width / 2, top = 54, bottom = height - 110, rowH = 24;
        ...
        for (int i = 0; i < sc.tabs.size(); i++) {
            ...
            if (y + 20 >= top && y <= bottom) {
                addDrawableChild(new DarkButton(cx - 100, y, 160, 20, Text.translatable("multichatwindows.tab.button", I18nUtil.tKeyOrLiteral(t.name)), () -> MinecraftClient.getInstance().setScreen(new TabEditScreen(this, serverKey, idx))));
                addDrawableChild(new DarkButton(cx + 64, y, 36, 20, Text.translatable(isAll ? "multichatwindows.locked" : "multichatwindows.delete"), () -> {
                    if (isAll) return;
        ...
        addDrawableChild(new DarkButton(cx - 100, height - 80, 200, 20, Text.translatable("multichatwindows.tabs.add"), () -> {
        ...
        addDrawableChild(new DarkButton(cx - 100, height - 54, 200, 20, Text.translatable("multichatwindows.save"), () -> { ConfigManager.saveServer(serverKey, sc); WindowService.rebuildForCurrentServer(); MinecraftClient.getInstance().setScreen(parent); }));
    ...
    @Override protected boolean onScroll(int delta) { scrollY += delta; init(); return true; }

    @Override
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        super.render(ctx, mouseX, mouseY, delta);
        ctx.drawCenteredTextWithShadow(textRenderer, Text.translatable("multichatwindows.tabs.title"), width / 2, 14, 0xFFFFFFFF);
        ctx.drawCenteredTextWithShadow(textRenderer, Text.translatable("multichatwindows.server.current", Text.literal(serverKey)), width / 2, 28, 0xFFB0B0B0);
    }
}
